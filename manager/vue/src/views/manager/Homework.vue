      ...
      <el-table :data="tableData" stripe>
        ...
        <el-table-column prop="file" label="作业文件">
          <template v-slot="scope">
            <el-button type="warning" size="mini" @click="down(scope.row.file)">下载</el-button>
          </template>
        </el-table-column>
        <el-table-column prop="score" label="作业打分"></el-table-column>
        <el-table-column prop="descr" label="评语"></el-table-column>
...
export default {
  ...
  methods: {
    loadCourse() {
      this.$request.get('/course/selectAll?studentId=' + this.user.id).then(res => {
        if (res.code === '200') {
    ...
    del(id) {   // 单个删除
      this.$confirm('确定删除作业吗？', '删除确认', {type: "warning"}).then(response => {
